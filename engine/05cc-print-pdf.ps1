...
Set-SageCurrentStep -Context $Context -Step "build_print_pdf" -Data @{
...
$BuildScriptPath = Join-Path $Context.EnginePath "05ca-print-docx.ps1"
$DocxPath = Join-Path $OutputRoot "$BookName`_print.docx"
$PdfPath = Join-Path $OutputRoot "$BookName`_print.pdf"
...
function Invoke-StableDocxBuild {
    ...
    if (!(Test-Path $ScriptPath)) {
        throw "05ca-print-docx.ps1 not found: $ScriptPath"
    }
    ...
    if ($ExitCode -ne 0) {
        throw "05ca-print-docx.ps1 failed with exit code $ExitCode."
    }
}
...
if (!(Test-Path $WorkspaceRoot)) {
    Fail-SageStep -Context $Context -Step "build_print_pdf" -Message "Workspace not found." -Data @{ workspace = $WorkspaceRoot; language = $LanguageCode }
...
try {
    ...
    if ((Test-Path $DocxPath) -and $_.Exception.Message -match "failed with exit code 1") {
        ...
        Fail-SageStep -Context $Context -Step "build_print_pdf" -Message "DOCX build prerequisite failed." -Data @{
...
if (!(Test-Path $DocxPath)) {
    Fail-SageStep -Context $Context -Step "build_print_pdf" -Message "DOCX output not found after build." -Data @{
        docx_output = $DocxPath
        language = $LanguageCode
    }
...
if (Test-Path $PdfPath) {
    ...
    $BackupFile = Join-Path $BackupRoot ("{0}_{1}{2}" -f ($BookName + "_print"), $Stamp, [System.IO.Path]::GetExtension($PdfPath))
    ...
    try {
        ...
        Fail-SageStep -Context $Context -Step "build_print_pdf" -Message "Existing PDF file is locked." -Data @{
            output = $PdfPath
...
try {
    ...
    Write-Host ""
    Write-Host "Print PDF build completed successfully:"
    Write-Host $PdfPath
}
catch {
    Fail-SageStep -Context $Context -Step "build_print_pdf" -Message "PDF build failed." -Data @{
        output = $PdfPath
...
Complete-SageStep -Context $Context -Step "build_print_pdf" -State "success" -Message "Print PDF build completed." -Data @{
    language = $LanguageCode
    output = $PdfPath
    source_docx = $DocxPath
    backup_output = $BackupFile
