...
Set-SageCurrentStep -Context $Context -Step "build-simple" -Data @{
    language = $LanguageCode
    auto_number = [bool]$AutoNumber
    generator = "python-docx-clean"
}
...
if (!(Test-Path $ChapterRoot)) {
    Fail-SageStep -Context $Context -Step "build-simple" -Message "Chapter root not found." -Data @{ chapter_root = $ChapterRoot; language = $LanguageCode }
...
if ($mdFiles.Count -eq 0) {
    Fail-SageStep -Context $Context -Step "build-simple" -Message "No markdown files found." -Data @{ chapter_root = $ChapterRoot; language = $LanguageCode }
    Write-Error "No markdown files found."
    exit 1
}

Write-Host "Found $($mdFiles.Count) chapter files for simple DOCX."
Write-Host ""
...
$OutputFile = Join-Path $OutputRoot "$BookName`_upload.docx"
...
if (Test-Path $OutputFile) {
    ...
    $BackupFile = Join-Path $BackupRoot ("{0}_{1}{2}" -f ($BookName + "_upload"), $Stamp, [System.IO.Path]::GetExtension($OutputFile))
    Copy-Item -LiteralPath $OutputFile -Destination $BackupFile -Force
    Write-Host ""
    Write-Host "Backed up existing simple DOCX to:"
    ...
    try {
        ...
        Fail-SageStep -Context $Context -Step "build-simple" -Message "Existing simple output file is locked." -Data @{
            output = $OutputFile
            backup_output = $BackupFile
            error = $_.Exception.Message
            language = $LanguageCode
        }
        Write-Error "Existing simple output file is locked. Please close the Word document and try again: $OutputFile"
        exit 1
    }
}

if (!(Test-Path $BuilderScript)) {
    Fail-SageStep -Context $Context -Step "build-simple" -Message "Builder script not found." -Data @{ builder = $BuilderScript; language = $LanguageCode }
...
if ($AutoNumber) {
    Write-Host ""
    Write-Host "Auto numbering is ignored in simple upload mode."
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Simple DOCX builder exited with code $LASTEXITCODE."
    }

    if (!(Test-Path $OutputFile)) {
        throw "Simple DOCX builder did not produce an output file."
    }

    Write-Host ""
    Write-Host "Simple DOCX build completed successfully:"
    Write-Host $OutputFile
}
catch {
    Fail-SageStep -Context $Context -Step "build-simple" -Message "Simple DOCX build failed." -Data @{
        output = $OutputFile
        error = $_.Exception.Message
        language = $LanguageCode
    }
    Write-Error "Simple DOCX build failed: $($_.Exception.Message)"
    exit 1
}

Complete-SageStep -Context $Context -Step "build-simple" -State "success" -Message "Simple upload DOCX build completed." -Data @{
    language = $LanguageCode
